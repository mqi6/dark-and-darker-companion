...
console.log(JSON.stringify({ itemAlias: plan.itemAlias, tabIndex: plan.tabIndex, sourceCell: plan.source.grid, destinationTabIndex: plan.tabIndex, destinationCell: plan.destination.grid, quantity: 1, footprint: "1x1", dragCount: 1, statement: "Exactly one left drag; no retry. Select Confirm Move or Cancel locally; no drag has been dispatched." }, null, 2));

...
const refreshPlan = JSON.parse(await readFile(resolve("fixtures-private/runtime/move-003-refresh/plan.private.json"), "utf8")) as { window: { clientBounds: StashGridCalibration["windowBounds"] }; gameBuildFingerprint: string };
...
const plan = prepareSupervisedMove({ request: { taskId: "MOVE-003", planId: "MOVE-003-plan-automatic-001", actionId: "MOVE-003-action-001", itemAlias: candidate.itemAlias, inventoryId: candidate.inventoryId, tabIndex: candidate.tabIndex, destination: candidate.destination, expectedSnapshotHash: snapshot.snapshotHash, expectedSnapshotVersion: snapshot.snapshotVersion }, projection, mapping, calibration: calibrationProfile.calibration, runtime: { runtimeProfileKey: mapping.runtimeProfileKey, availableInventoryIds: mapping.availableInventoryIds, selectedTabIndex: candidate.tabIndex, gameBuildFingerprint: refreshPlan.gameBuildFingerprint, windowBounds: refreshPlan.window.clientBounds, isForeground: true }, pageEnabled: true, reservedRegions: [] });
...
console.log(JSON.stringify({ itemAlias: plan.itemAlias, tabIndex: plan.tabIndex, inventoryId: plan.inventoryId, source: plan.source, destination: plan.destination, quantity: 1, footprint: "1x1", snapshotHashShort: plan.sourceSnapshotHash.slice(0, 12), snapshotVersion: plan.sourceSnapshotVersion, calibrationProfileId: plan.calibrationProfileId, gameBuildFingerprint: plan.gameBuildFingerprint, planFingerprint: plan.planFingerprint, previewPath: resolve(runtimeDirectory, "plan-preview.private.html"), statement: "Exactly one left drag; no retry. No drag has been authorized or dispatched." }, null, 2));

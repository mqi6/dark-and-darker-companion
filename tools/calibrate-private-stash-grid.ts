...
const includeBag = args["include-bag"] === "true";
...
try {
  ...
  const topLeft = captureCursor
    ? await capturePoint("stash outer top-left", prompt)
    : parsePoint(await prompt.question("Stash outer top-left screen point (x,y): "));
  const bottomRight = captureCursor
    ? await capturePoint("stash outer bottom-right", prompt)
    : parsePoint(await prompt.question("Stash outer bottom-right screen point (x,y): "));
  ...
  let bagCalibration: StashGridCalibration | undefined;
  if (includeBag) {
    console.log("Now enter the OUTER edges of the complete 10x5 character bag. No mouse input will be generated.");
    const bagTopLeft = captureCursor
      ? await capturePoint("character bag outer top-left", prompt)
      : parsePoint(await prompt.question("Character bag outer top-left screen point (x,y): "));
    const bagBottomRight = captureCursor
      ? await capturePoint("character bag outer bottom-right", prompt)
      : parsePoint(await prompt.question("Character bag outer bottom-right screen point (x,y): "));
    bagCalibration = createCharacterBagGridCalibration({
      profileId,
      gameBuildFingerprint,
      windowBounds: window.bounds,
      gridTopLeft: bagTopLeft,
      gridBottomRight: bagBottomRight
    });
  }
  ...
  await writeFile(
    resolve(outputDirectory, "calibration.private.json"),
  ...
  await writeFile(
    ...
    renderPreview(calibration, "12x20 stash", source, destination),
    "utf8"
  );
  if (bagCalibration) {
    await writeFile(
      resolve(outputDirectory, "bag-calibration-preview.private.html"),
      renderPreview(bagCalibration, "10x5 character bag"),
      "utf8"
    );
  }
  ...
  console.log(
    bagCalibration
      ? "Stash and bag previews are non-clicking and generated no game input."
      : "The stash preview is non-clicking and generated no game input."
  );
} finally {
  prompt.close();
}
...
async function capturePoint(
  ...
  await prompt.question(
    `Press Enter, then return to the game and place the cursor on the ${label} grid edge within 5 seconds. No click: `
  );
...
function renderPreview(
  ...
  const marker = (point: ScreenPoint | undefined, markerLabel: string, color: string) => {
    if (!point) return "";
    ...
    return `<circle cx="${localX}" cy="${localY}" r="${Math.min(calibration.cellWidth, calibration.cellHeight) * 0.35}" fill="${color}" fill-opacity="0.65"/><text x="${localX + 6}" y="${localY - 6}" fill="${color}">${markerLabel} (${point.x},${point.y}) center (${center.x.toFixed(1)},${center.y.toFixed(1)})</text>`;
  };
  const vertical = Array.from(
    { length: calibration.grid.columns + 1 },
    (_, x) => `<line x1="${x * calibration.cellWidth}" y1="0" x2="${x * calibration.cellWidth}" y2="${height}"/>`
  ).join("");
  const horizontal = Array.from(
    { length: calibration.grid.rows + 1 },
    (_, y) => `<line x1="0" y1="${y * calibration.cellHeight}" x2="${width}" y2="${y * calibration.cellHeight}"/>`
  ).join("");
  return `<!doctype html><meta charset="utf-8"><title>Private ${label} calibration preview</title><style>body{background:#151515;color:#eee;font:14px system-ui}svg{background:#222;border:3px solid #fff}line{stroke:#777;stroke-width:.5}text{font-size:12px;font-weight:700}</style><h1>Private non-clicking ${label} preview</h1><p>Complete calibrated ${calibration.grid.columns}x${calibration.grid.rows} boundary. Do not publish: screen coordinates are private.</p><svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">${vertical}${horizontal}${marker(source, "SOURCE", "#ffcc00")}${marker(destination, "DESTINATION", "#00e5ff")}</svg>`;
}

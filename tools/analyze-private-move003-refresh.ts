...
import { projectSpatialState, RECTANGULAR_STORAGE_INVENTORY_IDS } from "../src/domain/inventoryGeometry";
...
if (freshCharacterCandidates.length > 0) {
  ...
  const requiredIds = new Set<number>(RECTANGULAR_STORAGE_INVENTORY_IDS);
  const requiredContainers = spatial.containers.filter(container => requiredIds.has(container.inventoryId));
  const diagnostics = requiredContainers.flatMap(container => container.diagnostics);
  spatialValidation = { ready: requiredContainers.length === RECTANGULAR_STORAGE_INVENTORY_IDS.length && requiredContainers.every(container => container.status === "ready"), blockedContainerCount: requiredContainers.filter(container => container.status === "blocked").length, diagnosticCounts: Object.fromEntries([...new Set(diagnostics.map(value => value.code))].sort().map(code => [code, diagnostics.filter(value => value.code === code).length])) };
}
const review = {
  reviewVersion: 1, sampleId: "MOVE-003-REFRESH", sanitized: true,
...
if (freshCharacterCandidates.length > 0) {
  const privateState = await new GameStateReducer(localizationCatalogSchema.parse(localizationJson), "move003-refresh-001").replaceBaseline(freshCharacterCandidates);
  await writeFile(resolve(directory, "semantic-snapshot.sanitized-private.json"), `${JSON.stringify(privateState.protocol, null, 2)}\n`, "utf8");
}
